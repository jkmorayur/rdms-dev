<?php
  ...
  class crone extends CI_Controller {
     ...
     function sendsms() {
          ...
          $route = 2;
          $enqs = $this->db->select('enq_id, enq_cus_name, enq_cus_mobile')->limit(300)->get_where('cpnl_enquiry', array('enq_current_status' => 1, 'is_exe'  => 0))->result_array();
          ...
          //      $msg = "Dear Customer,  Wishing you a joyous Christmas and a prosperous New Year, 918129909090  - Royal Drive South India's Largest Pre-Owned Luxury Car Showroom";
     ...
     function rdownvehicleinsurancerenewal() {
          ...
          $return = $this->db->select($selArray, false)
                  ...
                  ->order_by('val_insurance_comp_date')->where('val_comp_stock', 1)->where('((DATEDIFF(val_insurance_comp_date, CURRENT_DATE) -1) <= 3)')
          ...
          if(!empty($return) && !empty($to)) {
               foreach ($return as $key => $value) {
                    $template = $value['val_veh_no'] . ', '. $value['brd_title'] . ', ' . $value['mod_title'] . ' insurance due for renewal on ' . date('d-m-Y', strtotime($value['val_insurance_comp_date']));
                    ...
                    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                    curl_setopt($ch, CURLOPT_POST, 1);
                    curl_setopt($ch, CURLOPT_POSTFIELDS, "ver=1&mode=1&action=push_sms&type=1&route=" . $route . "&login_name=" . $username . "&api_password=" . 
     ...
     function insurance() {
          ...
          //         ->join($this->tbl_brand, $this->tbl_brand . '.brd_id = ' . $this->tbl_valuation . '.val_brand', 'left')
          //         ->join($this->tbl_variant, $this->tbl_variant . '.var_id = ' . $this->tbl_valuation . '.val_variant', 'left')
          //         ->where("(DATEDIFF(DATE(" . $this->tbl_valuation . ".val_insurance_comp_date), '" . date('Y-m-d') . "') <= 4) OR (DATEDIFF(DATE(" . $this->tbl_valuation . ".val_insurance_ll_date), '" . date('Y-m-d') . "') <= 4)")
          //         ->where_in($this->tbl_valuation . '.val_status', array(39, 40))->order_by('val_insurance_validity')->where('val_comp_stock', 1)
          //         ->get($this->tbl_valuation)->result_array();
          //         debug($return);
       ...
       function sendtestsms() {
          ...
          foreach ($numbers as $key => $value) {
               ...
               $msg = "Dear Customer Royal Drive Showrooms, will closed on SEP 3,4 2022 due to Annual Meet and Tour - Royal Drive South India's Largest Pre-Owned Luxury Car Showroom";
       ...
       function send_whatsapp_msg($to_number='') {//Whatsapp Cloud Api test
          ...
          curl_setopt_array($curl, array(
            ...
            CURLOPT_HTTPHEADER => array(
              ...
              'Authorization: Bearer EAAO9dLJM5FgBAKMfAjmFf60M15STEj4RCuhKIRdzsza17F0BzuD1GVK1f3DgIaC15M6bTw76uTabtpGDJM5ycosXVcsBaEbJK9fPfJtcw0OVh4Gf93ZBmSjHiZAaH2ElvqhP08fBAZB2u4s09ZBhydysXIUXfj6H0ZAk583nL7CV80HHbGeyKBDqXQF350lTr6f1CB7P3ay6oJxBPa2ZByBy1W0puewOQZD'
            ),
          ));
     ...
     function nullnextfolloup() {
          $f = $this->db->query("SELECT enq_id FROM `cpnl_enquiry` WHERE enq_added_on > DATE('2022-08-01')")->result_array();
          //debug($f);
          foreach ($f as $key => $value) {
               $fol = $this->db->query("SELECT foll_cus_id FROM `cpnl_followup` WHERE foll_cus_id = " . $value['enq_id'])->result_array();
               if(empty($fol)) {
                    debug($value, 0);
               }
          }
     ...
     function forceLost() {
          $f = $this->db->query("SELECT cpnl_dar_followup.*, cpnl_followup.foll_id, cpnl_enquiry.enq_cus_name, cpnl_enquiry.enq_cus_mobile, cpnl_enquiry.enq_id,
     ...
     function removedisable() {
         error_reporting(E_ALL);
         $mydir = FCPATH . 'assets' . DIRECTORY_SEPARATOR . 'uploads' . DIRECTORY_SEPARATOR . 'product' . DIRECTORY_SEPARATOR;
         $files = scandir($mydir);
         unset($files[0]);
         unset($files[1]);
         ...
         echo "<pre>" . implode("\n", $prefixed_array) . "</pre>";
          ...
          foreach ($f as $key => $value) {          
              $images = $this->db->select('pdi_image')->where('pdi_prod_id', $value['prd_id'])->get('rana_prod_images')->result_array();
              if(!empty($images)) {
                //   debug($images, 0);
     ...
     function revertAssignedEnq() {
          //$f = $this->db->query("SELECT log_ref_id FROM `cpnl_general_log` WHERE `log_controller` LIKE '%quk-assign-inquiry-%' AND log_added_on_in LIKE '%2022-10-15%'")->result_array();
          $f = $this->db->query("SELECT foll_id FROM `cpnl_followup` WHERE `foll_remarks` LIKE '%JAMSHEER\'s enquires reassigned%' AND foll_entry_date LIKE '%2022-10-15%'")->result_array();
               exit;
          foreach ($f as $key => $value) {
      ...
      function pool() {
          $f = $this->db->query("SELECT * FROM `cpnl_general_log` WHERE `log_controller` LIKE '%quk-assign-inquiry%' AND log_added_on_in LIKE '%2022-10-31%'")->result_array();
          foreach ($f as $key => $value) {
     ...
     function getLastFollowupDate(){
          ...
          $enqs = $this->db->select('enq_id')->limit(150)->order_by('enq_id', 'DESC')->where('(enq_last_foll_cust_rmk IS NULL AND enq_last_foll_date IS NULL)')
                           ->get_where('cpnl_enquiry', array('is_exe' => 0))->result_array();
          
          if(!empty($enqs)) {
               foreach ($enqs as $key => $value) {
                    //get last followup date
                    $lstFoll = $this->db->select('foll_entry_date, foll_added_by, foll_budget_from, foll_budget_to')->where(array('foll_cus_id' => $value['enq_id'], 'foll_is_cmnt' => 0))
                                   ->order_by('foll_id', 'DESC')->limit(1)->get($this->tbl_followup)->row_array();

                    $lstCustRem = $this->db->select('foll_customer_feedback')->where(array('foll_cus_id' => $value['enq_id'], 'foll_is_cmnt' => 0, 'foll_customer_feedback !=' => 'NULL'))
                              ->order_by('foll_id', 'DESC')->limit(1)->get($this->tbl_followup)->row_array();
                    ...
                    $data['enq_last_foll_cust_rmk'] = isset($lstCustRem['foll_customer_feedback']) ? $lstCustRem['foll_customer_feedback'] : NULL;
                    $data['is_exe'] = 1;
     ...
     function careinv() {
          ...
          $enqs = $this->db->select('enq_id, enq_cus_name, enq_cus_mobile,enq_showroom_id')->limit(100)
               ->order_by('enq_showroom_id', 'DESC')->get_where('cpnl_enquiry', array('enq_current_status' => 1, 'is_exe'  => 0))->result_array();
...
With great pleasure, we announce the opening ceremony of Royal Drive Care – Premium Luxury Car Service, an initiative by Royal Drive on 3rd March 2023 at 3 pm. Meet our expert technicians and see our state-of-the-art facility. We welcome you to be a part of history in making. bit.ly/rd-care";
     ...
     function bookingid() {
          ...
          foreach ($f as $key => $value) {
               $dcod = $this->db->join($this->tbl_divisions, $this->tbl_divisions.'.div_id = ' . $this->tbl_showroom . '.shr_division')
               ->get_where($this->tbl_showroom, array('shr_id' => $value['vbk_showroom']))->row()->div_code . $value['vbk_ref_no'];
               $this->db->where('vbk_id', $value['vbk_id'])->update($this->tbl_vehicle_booking_master, array('vbk_ref_no' => $dcod));
          }
     }
     ...
     function enquirymeta() {
          ...
          $f = $this->db->select('enq_id')->where('enq_added_to_meta = 0')->limit(200)->get($this->tbl_enquiry)->result_array();
          foreach ($f as $key => $value) {
               if(!empty($f)) {
                    $update = array(); 
                    $update['enqm_enq_id'] = $value['enq_id'];
                    $sales = $this->db->select($selectArray)
                         ...
                         ->order_by($this->tbl_vehicle . '.veh_id', 'DESC')->limit(1)->get_where($this->tbl_vehicle, array(
                              ...
                              $this->tbl_vehicle . '.veh_status' => 1))->row_array();
                    if(!empty($sales)) {
                         $update['enqm_sls_veh'] = $sales['brd_title'] . ', ' . $sales['mod_title'] . ', ' . $sales['var_variant_name'];
                    } 
                    $purchase = $this->db->select($selectArray)
                         ...
                         ->order_by($this->tbl_vehicle . '.veh_id', 'DESC')->limit(1)->get_where($this->tbl_vehicle, array(
                              $this->tbl_vehicle . '.veh_enq_id' => $value['enq_id'],
                              $this->tbl_vehicle . '.veh_status' => 2))->row_array();
                    if(!empty($purchase)) {
                         $update['enqm_pur_veh'] = $purchase['brd_title'] . ', ' . $purchase['mod_title'] . ', ' . $purchase['var_variant_name'];
                    }
                    $this->db->insert('cpnl_enquiry_meta', $update);
                    $this->db->where('enq_id', $value['enq_id'])->update('cpnl_enquiry', array('enq_added_to_meta' => 1));
               }
          }
          ...
          foreach ($f as $key => $value) {
               if(!empty($f)) {
                    ...
                    if(isset($lstFoll['foll_budget']) && !empty($lstFoll['foll_budget'])) {
                         debug($lstFoll, 0);
                         $this->db->where('enq_id', $value['enq_id'])->update($this->tbl_enquiry, array('enq_budget_id' => $lstFoll['foll_budget']));
                    }
     ...
     function refurbactualcose() {
          ...
          exit;
          $f = $this->db->query("SELECT upgrd_master_id, COALESCE(SUM(`upgrd_value`), 0) AS upgrd_value FROM `cpnl_valuation_upgrade_details` 
                                 GROUP BY `upgrd_master_id`  HAVING upgrd_value > 0 ORDER BY upgrd_master_id ASC")->result_array();
                                     
          if(!empty($f)) {
               foreach ($f as $key => $value) {
                    $this->db->where('val_id', $value['upgrd_master_id'])->update($this->tbl_valuation, array('val_refurb_cost' => $value['upgrd_value']));
               }
               echo 'Compleated!';
          } else {
     ...
     function stockdate() {
          $f = $this->db->select('val_id, val_purchased_date')->where('val_purchased_date IS NULL')->order_by('val_id', 'DESC')->limit(200)->get('cpnl_valuation')->result_array();
          foreach ($f as $key => $value) {
               //
