<?php
  ...
  class Home_model extends CI_Model {
       ...
       public function __construct() {
            ...
            $this->rbl_referel = TABLE_PREFIX_PORTAL . 'referel';
       ...
       function getLuxuryBrandsLog($id = '') {
            ...
            $this->db->order_by('brd_title');
            $this->db->where('brd_logo <> ""');
            $this->db->where('brd_section', 1);
            $categories1 = $categories2 = $this->db->get()->result_array();
            return array_merge($categories1, $categories2);
       }
       
       function getVehicle($id = '', $limit = 4) {
            $this->db->select($this->tbl_products . '.*, IF(prd_show_price = 1, prd_price, 0) AS prd_price,' . $this->tbl_brands . '.*,' . $this->tbl_model . '.*,' . $this->tbl_variant . '.*', false)->from($this->tbl_products);
       ...
       /*function newArrivals() {
            ...
            $this->db->where($this->tbl_products . '.prd_status', 1);
            $this->db->where($this->tbl_products . '.prd_latest', 1);
            $this->db->where($this->tbl_products . '.prd_booked', 1);
            $this->db->where($this->tbl_products . '.prd_added_by_user', 0);
            $productsBkd = $this->db->get()->result_array();
            ...
            $this->db->where($this->tbl_products . '.prd_status', 1);
            $this->db->where($this->tbl_products . '.prd_latest', 1);
            $this->db->where($this->tbl_products . '.prd_soled', 1);
            $this->db->where($this->tbl_products . '.prd_added_by_user', 0);
            $productsSld = $this->db->get()->result_array();
            
            $productsArray = array_merge($productsNoBkdSld, $productsBkd, $productsSld);
            $products['product_details'] = array();
            if (!empty($productsArray)) {
                 foreach ($productsArray as $key => $value) {
                      $prodSpecifications = $this->db->order_by("spe_id", "asc")->get_where($this->tbl_products_specification, array('spe_prod_id' => $value['prd_id']))->result_array();
...
//            $this->db->order_by($this->tbl_products . '.prd_price', 'desc');
       ...
       function referFriend($data) {
            if (!empty($data)) {
                 ...
                 $data['ref_added_on'] = date('Y-m-d H:i:s');
                 $data['ref_referel_number'] = time();
       ...
       function vehiclesCount($category) {
          if ($category == 'new') {
                $query = $this->db->join($this->tbl_model, $this->tbl_model . '.mod_id = ' . $this->tbl_products . '.prd_model', 'left')->where('prd_latest', 1)->where('prd_rd_mini', 0)->where('prd_status', 1)->where($this->tbl_model . '.mod_is_ev', 0)->get($this->tbl_products);
               return $query->num_rows();
          }
          if ($category == 'popular') {
               $query = $this->db->join($this->tbl_model, $this->tbl_model . '.mod_id = ' . $this->tbl_products . '.prd_model', 'left')->where('prd_popular', 1)->where('prd_rd_mini', 0)->where('prd_status', 1)->where($this->tbl_model . '.mod_is_ev', 0)->get($this->tbl_products);
               return $query->num_rows();
          }
          if ($category == 'rdMini') {
           ...
           if ($category == 'ev') {
               $query = $this->db->join($this->tbl_model, $this->tbl_model . '.mod_id = ' . $this->tbl_products . '.prd_model', 'left')->where('prd_rd_mini', 0)->where($this->tbl_model . '.mod_is_ev', 1)->where('prd_status', 1)->get($this->tbl_products);
               return $query->num_rows();
          }
     }
