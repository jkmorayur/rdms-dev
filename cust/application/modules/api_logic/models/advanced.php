...
class advanced extends CI_Model
{
     ...
     public function __construct()
     {
          ...
          $this->tbl_users_cpnl = TABLE_PREFIX_PORTAL . 'users ';
          $this->tbl_careers = TABLE_PREFIX_PORTAL . 'careers';
            ...
            $this->tbl_subscriptions = TABLE_PREFIX_PORTAL . 'subscriptions ';
   ...
   function updateuser($data)
     ...
     function sellYourVeh($params)
     {
//$params['phone_number']
          $userDetails = $this->db->select(array('id'))
               ->where(array('phone' =>$params['customer']['phone_number']))->get(rana_users)->row_array();
          if (!empty($userDetails)) {
              // $this->db->where('phone', $phone)->update(rana_users, array('rusr_otp' => null, 'rusr_phone_verified' => 1, 'active' => 1));
              //$this->db->insert(rana_users, array('usr_phone_code' => $cntrCode, 'phone' => $number, 'rusr_otp' => $otp, 'active' => 0));
              //$userId = $this->db->insert_id();
            //  return array('id' => $userId, 'otp' => $otp);
            $params['customer']['user_id']=$userDetails['id'];
         //  debug( $params);
     ...
     function addProduct($params) {
          ...
          $datas['basicinfo']['prd_user_id'] = $params['customer']['user_id'];//pone_
          $datas['basicinfo']['prd_number'] = gen_random();
          $datas['basicinfo']['prd_brand'] = $params['vehicle_info']['brand_id'];
          $datas['basicinfo']['prd_model'] = $params['vehicle_info']['model_id'];
          $datas['basicinfo']['prd_variant'] = $params['vehicle_info']['variant_d'];
          $datas['basicinfo']['prd_price'] = $params['vehicle_info']['price'];
          $datas['basicinfo']['prd_year'] = $params['vehicle_info']['purchase_year'];
          $datas['basicinfo']['prd_fual'] = $params['vehicle_info']['fuel_type_id'];
          $datas['basicinfo']['prd_transmission'] = $params['vehicle_info']['transmission_id'];
          $datas['basicinfo']['prd_km_run'] = $params['vehicle_info']['kms_driven'];

          $this->db->insert($this->tbl_products, $datas['basicinfo']);
          $prdId = $this->db->insert_id();
          if (isset($params['vehicle_info']['images']) && !empty($params['vehicle_info']['images'])) {
               foreach ($params['vehicle_info']['images'] as $tkey => $timgId) {
                    $img_default= $tkey==0?1:0;
                    if (!empty($images)) {
                         $imgs = array(
                             'pdi_prod_id' => $prdId,
                             'pdi_image' => $images['img'],
                             'pdi_is_default' => $img_default
                         );
     ...
     function saveSubscription($params) {
         // debug($params['email']);
          if (isset($params['email'])) {
                    ...
                    $res=$this->db->insert($this->tbl_subscriptions, $data);
                    return true;
               }
