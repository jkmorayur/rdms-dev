  ...
  class Vehicle_model extends CI_Model {
       ...
       public function getVehicle($id) {
            ...
            if (!empty($id)) {
                 ...
                 $this->db->select($this->tbl_products . '.*, IF(prd_show_price = 1, prd_price, 0) AS prd_price, ' . TABLE_PREFIX . 'brand.*,' . 
                              $this->tbl_model . '.*,' . $this->tbl_variant . '.*,' . $this->tbl_vehicle_colors . '.vc_color AS prd_color', false)->from($this->tbl_products);
                 ...
                 $this->db->join($this->tbl_vehicle_colors, $this->tbl_vehicle_colors . '.vc_id = ' . $this->tbl_products . '.prd_color_id', 'left');
       ...
       public function getRelatedVehicles($id) {
            if (!empty($id)) {
                 ...
                 $rdmini = isset($vehDetails['prd_rd_mini']) ? $vehDetails['prd_rd_mini'] : 0;

                 $this->db->select($this->tbl_products . '.*, IF(prd_show_price = 1, prd_price, 0) AS prd_price, ' . TABLE_PREFIX . 'brand.*,' . $this->tbl_model . '.*,' . $this->tbl_variant . '.*', false)->from($this->tbl_products);
                 $this->db->join(TABLE_PREFIX . 'brand', TABLE_PREFIX . 'brand.brd_id = ' . $this->tbl_products . '.prd_brand', 'left');
                 ...
                 //if ($rdmini == 1) {
                      $this->db->where($this->tbl_products . '.prd_rd_mini', $rdmini);
                 //}
                 $productsArray = $this->db->get()->result_array();
                 $products = array();
                 if (!empty($productsArray)) {
