...
class Ion_auth_model extends CI_Model {
     ...
     public function __construct() {
         ...
         // $this->tables = $this->config->item('tables', 'ion_auth_rdms_api');
         $this->tables = $this->config->item('tables_api', 'ion_auth');//jsk 

          //initialize data
        //  $this->identity_column = $this->config->item('identity', 'ion_auth');
        $this->identity_column = $this->config->item('identity_api', 'ion_auth');
      ...
      * @author Mathew
      * */
     public function login($identity, $password, $remember = FALSE) {//old
         // debug($this->identity_column);
         // debug($this->tables);
          //debug($this->tables['users']);
          ...
          // $query = $this->db->select($this->identity_column . ', usr_username,usr_first_name,usr_last_name,usr_phone,usr_email,usr_id,usr_api_key,usr_showroom, usr_password, usr_division, '
          //                         . 'usr_active, usr_last_login, usr_avatar, usr_appdownloadlink, ' .
          //                         $this->tables['user_access'] . '.cua_access AS user_access,' .
          //                         TABLE_PREFIX_PORTAL . 'users_groups.group_id,' . TABLE_PREFIX_PORTAL . 'groups.grp_slug AS grp_slug')
          //                 ->join($this->tables['user_access'], $this->tables['user_access'] . '.cua_group_id = ' . $this->tables['users'] . '.usr_id', 'left')
          //                 ->join(TABLE_PREFIX_PORTAL . 'users_groups', TABLE_PREFIX_PORTAL . 'users_groups.user_id = ' . $this->tables['users'] . '.usr_id', 'left')
          //                 ->join(TABLE_PREFIX_PORTAL . 'groups', TABLE_PREFIX_PORTAL . 'groups.id = ' . TABLE_PREFIX_PORTAL . 'users_groups.group_id', 'left')
          //                 ->where($this->identity_column, $this->db->escape_str($identity))->limit(1)->get($this->tables['users']);
                         //  echo $this->db->last_query();
                         //  exit;
                         $query = $this->db->select($this->identity_column . ', usr_username,,usr_first_name,usr_last_name,usr_api_key,usr_showroom, usr_division,usr_phone,usr_email, usr_id, usr_password, usr_active, usr_last_login, usr_avatar, usr_appdownloadlink, ' .
                         $this->tables['user_access'] . '.cua_access AS user_access,' .
                         TABLE_PREFIX_PORTAL . 'users_groups.group_id,' . TABLE_PREFIX_PORTAL . 'groups.grp_slug AS grp_slug,' . TABLE_PREFIX_PORTAL . 'designation.desig_slug')
                 ->join($this->tables['user_access'], $this->tables['user_access'] . '.cua_group_id = ' . $this->tables['users'] . '.usr_id', 'left')
                 ->join(TABLE_PREFIX_PORTAL . 'users_groups', TABLE_PREFIX_PORTAL . 'users_groups.user_id = ' . $this->tables['users'] . '.usr_id', 'left')
                 ->join(TABLE_PREFIX_PORTAL . 'groups', TABLE_PREFIX_PORTAL . 'groups.id = ' . TABLE_PREFIX_PORTAL . 'users_groups.group_id', 'left')
                 ->join(TABLE_PREFIX_PORTAL . 'designation', TABLE_PREFIX_PORTAL . 'designation.desig_id = ' . $this->tables['users'] . '.usr_designation_new', 'left')
  ...
  return $data['user']=$user;
     ...
     public function login_jq($identity, $password, $remember = FALSE) {//new
         // debug(TABLE_PREFIX_PORTAL);
          //debug($this->identity_column); rd Api
          //debug($this->tables['user_access']);
            $this->trigger_events('pre_login');
            if (empty($identity) || empty($password)) {
                 $this->set_error('login_unsuccessful');
                 return FALSE;
            ...
            $query = $this->db->select($this->identity_column . ', usr_username, usr_showroom, usr_division, usr_email, usr_id, usr_password, usr_active, usr_last_login, usr_avatar, usr_appdownloadlink, ' .
                            $this->tables['user_access'] . '.cua_access AS user_access,' .
                            TABLE_PREFIX_PORTAL . 'users_groups.group_id,' . TABLE_PREFIX_PORTAL . 'groups.grp_slug AS grp_slug,' . TABLE_PREFIX_PORTAL . 'designation.desig_slug')
                    ->join($this->tables['user_access'], $this->tables['user_access'] . '.cua_group_id = ' . $this->tables['users'] . '.usr_id', 'left')
                    ->join(TABLE_PREFIX_PORTAL . 'users_groups', TABLE_PREFIX_PORTAL . 'users_groups.user_id = ' . $this->tables['users'] . '.usr_id', 'left')
     ...
     public function loginNew($identity, $password, $remember = FALSE) {
          $this->trigger_events('pre_login');
     ...
     {
          //return $this->tables;$this->tables  cpnl_user_access
          if (!empty($id)) {
               return $this->db->select('cua_access')->where('cua_user_id', $id)->get('cpnl_user_access')->row_array();
          } else {
               return false;
          }
     }
