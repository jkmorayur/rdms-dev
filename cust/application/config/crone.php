...
India's Largest Pre-Owned Luxury Car Showroom";
       ...
       function rdownvehicleinsurancerenewal() {
            ...
            $return = $this->db->select($selArray, false)
                            ...
                            ->order_by('val_insurance_comp_date')->where('val_comp_stock', 0)->limit(1)->where('((DATEDIFF(val_insurance_comp_date,
CURRENT_DATE) -1) <= 3)')->get($this->tbl_valuation)->result_array();
...
//                '919645269090', // Soumya
    ...
    renewal on ' . date('d-m-Y', strtotime($value['val_insurance_comp_date']));
                      ...
                      curl_setopt($ch, CURLOPT_URL, "https://api.xpresssms.in/api/v2/SendSMS?");
                      curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                      curl_setopt($ch, CURLOPT_POST, 1);
                      curl_setopt($ch, CURLOPT_POSTFIELDS, "ApiKey=AzsdSFt21JOEHYuUcpD0LRqd88dYPY1wlkT0EgXiYJc=&ClientId=47d5c8bf-817d-4c5b-9c57-542ebbd41434"
                              . "&SenderId=" . $senderid . "&Message=" . $msg . "&MobileNumbers=" . $to . "&principleEntityId=100000000042909&templateId=" . $tmpId);
                      $buffer = curl_exec($ch);
                      curl_close($ch);
                      echo $buffer . '<br>';
                 }
