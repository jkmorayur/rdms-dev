...
class evaluation_model extends CI_Model
{
     ...
     function evaluation_ajax($postDatas, $filterDatas)
     {
          ...
          if (!empty($searchValue)) {
               $this->db->where("(val_veh_no LIKE '%" . $searchValue . "%' OR shr_location LIKE '%" . $searchValue . "%' OR "
                    . "brd_title LIKE '%" . $searchValue . "%' OR mod_title LIKE '%" . $searchValue . "%' OR enq_cus_name LIKE '%" . $searchValue . "%' OR enq_cus_mobile LIKE '%" . $searchValue . "%' OR enq_number LIKE '%" . $searchValue . "%') ");
          }
     ...
     function stock_ajax($postDatas, $filterDatas)
     {
          ...
          $columnSortOrder = isset($postDatas['order'][0]['dir']) ? $postDatas['order'][0]['dir'] : ''; // asc or desc
          $searchValue = isset($postDatas['search']['value']) ? $postDatas['search']['value'] : ''; // Search value
          $stsWhere = "(val_status = " . add_stock . " OR val_status = 28 OR val_status = 13)";
     ...
     function updateEvaluation($id, $data)
     {
          ...
          if (!empty($data)) {
               ...
               $data['val_minif_month'] = (isset($data['val_minif_month']) && !empty($data['val_minif_month'])) ? $data['val_minif_month'] : 0;
          ...
          if (!empty($data)) {
               ...
               for ($i = 0; $i < $count; $i++) {
                    ...
                    $this->db->insert($this->tbl_valuation_upgrade_details, array(
                         ...
                         'upgrd_added_by' => $this->uid, 'upgrd_added_on' => date('Y-m-d H:i:s')
                    ));
               }
          }
     }
                    ...
                    if (isset($datas['newRefrubishjob']['refurb_job'][$i])) {
                         $this->db->insert(
                              ...
                              array(
                                   ...
                                   'upgrd_is_done' => 1, 'upgrd_added_by' => $this->uid, 'upgrd_added_on' => date('Y-m-d H:i:s')
                              )
                         );
                    }
     ...
     {
          if (!empty($id)) {
               $EvaluationVehicle = $this->db->select($this->tbl_valuation . '.val_id,' . $this->tbl_valuation . '.val_stock_num,' . $this->tbl_valuation . '.val_prt_1,' . $this->tbl_valuation . '.val_veh_no,' . $this->tbl_valuation . '.val_cust_name,' . $this->tbl_valuation . '.val_rc_owner,' . $this->tbl_valuation . '.val_chasis_no,' . $this->tbl_model . '.*,' .
                    ...
                    $this->tbl_users . '.usr_first_name,' . $this->tbl_showroom . '.*,' . $this->tbl_divisions . '.div_name,' . $this->tbl_divisions . '.div_short_code,' .
                    'slsOfficer.usr_username AS so_usr_username, slsOfficer.usr_first_name AS so_usr_first_name,' .
                    'evtr.usr_first_name AS evtr_first_name, evtr.usr_last_name AS evtr_last_name,' . $this->tbl_banks . '.*')
                    ->join($this->tbl_model, $this->tbl_model . '.mod_id = ' . $this->tbl_valuation . '.val_model', 'left')
                    ->join($this->tbl_brand, $this->tbl_brand . '.brd_id = ' . $this->tbl_valuation . '.val_brand', 'left')
     ...
     function getPurchase_check_list($masterId = '')
     {
          ...
          if ($masterId) {
               $this->db->select($this->tbl_purchase_check_list . '.*,' . $this->tbl_valuation . '.val_prt_1,' .
                    $this->tbl_valuation . '.val_prt_2,' . $this->tbl_valuation . '.val_prt_3,' . $this->tbl_valuation . '.val_prt_4')
     ...
     function getDochistry($valId)
     {
          ...
          return $this->db->select($selectArray)
               ...
               ->get($this->tbl_valuation_doc_history)->result_array();
     }
