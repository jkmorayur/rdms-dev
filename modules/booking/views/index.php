                            ...
                            <tr>
                                ...
                                <td>
                                    <select multiple="multiple" style="float: left;width: auto;" class="cmbMultiSelect select2_group form-control" name="status[]">
                                        <option value="<?php echo confm_book; ?>" <?php echo (@in_array(confm_book, $sts)) ? 'selected="selected"' : ''; ?>>Booking confirmed</option>

                                        <option value="<?php echo vehicle_booked; ?>" <?php echo (@in_array(vehicle_booked, $sts)) ? 'selected="selected"' : ''; ?>>Vehicle booked</option>

                                        <option value="<?php echo book_delvry; ?>" <?php echo (@in_array(book_delvry, $sts)) ? 'selected="selected"' : ''; ?>>Vehicle delivered</option>
                                    </select>
                                </td>
                            </tr>
                            <tr>
...
<script>
    ...
    function empList(canDelete, frmData) {
            ...
            "columns": [{
                ...
                }, // Booking ID
                {
                    ...
                    "render": function(data, type, full, meta) {
                        var vbk_added_on = (full.vbk_added_on !== null && full.vbk_added_on !== '') ? new Date(full.vbk_added_on) : null;
                        if (vbk_added_on) {
                ...
                }, // Registration
                {
                    "data": "enq_number",
                    "render": function(data, type, full, meta) {
                        return '<a href="https://rdms.royaldrive.in/index.php/enquiry/printTrackCard/' + full.enq_id + '">' + full.enq_number + '</a>';
                    }
                    //"data": "enq_number"
                }, // Enq No
                {
                ...
                }, // RC Transfer address
                {
                    ...
                    "sortable": true,
                    "render": function(data, type, full, meta) {
                        var vbk_expect_delivery = (full.vbk_expect_delivery !== null && full.vbk_expect_delivery !== '') ? new Date(full.vbk_expect_delivery) : null;
                ...
                <?php if ($this->uid == 100) : ?> {
                        "render": function(data, type, full, meta) {
                            var deleteUrl = '<?php echo site_url("booking/permenentdeletebooking/"); ?>/' + full.vbk_id;
                            var content = '<a class="btnRemoveTableRow" href="javascript:void(0);" data-url="' + deleteUrl + '"><i title="View document" class="fa fa-trash"></i></a>';
                            return content;
                        }
                    },
                <?php endif; ?>
                <?php if (check_permission('booking', 'editbooking')) : ?> {
                        "render": function(data, type, full, meta) {
                            ...
                            var content = '<a href="' + editUrl + '"><i title="Update booking" class="fa fa-pencil"></i></a>';
                            return content;
                        }
                    }
                <?php endif; ?>
        ...
        $('#tblBooking tbody').on('click', 'tr', function(e) {
            ...
            var url = "<?php echo site_url('booking/bookingDetails/'); ?>/" + "<?php echo encryptor('" + var_id + "'); ?>";
