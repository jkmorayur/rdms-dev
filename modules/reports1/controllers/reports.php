...
class reports extends App_Controller
{
     ...
     function export_excel_drop_loss()
     {
          ...
          $heading = array('Enquiry Number', 'Customer', 'Customer Contact No', 'Districts', 'Vehicle', 'Type', 'Showroom', 'Sales Staff', 'Drope/Lost date', 'Last followup comment', 'Mode of contact', 'Customer status', 'Enquiry date');
          ...
          if (!empty($data['data'])) {
               foreach ($data['data'] as $key => $value) {
                    ...
                    $objPHPExcel->getActiveSheet()->setCellValue('M' . $row, date('d-m-Y', strtotime($value['enq_entry_date'])));
                    $row++;
                    $no++;
               }
          }
     ...
     function exportEnquires()
     {
          ...
          $objPHPExcel->getActiveSheet()->setTitle('Enquires report');
          $heading = array(
               'SL', 'Enq Date', 'Customer', 'Customer Number', 'Profession', 'Location', 'Vehicle',
               'Budget', 'Year', 'KM', 'Mode of enquiry', 'Colour Pref', 'Present Sales Staff', 'Team Leader',
               'Branch', 'Loan %', 'Loan AMT', 'Loan EMI', 'Loan period (year)', 'Exchange', 'Customer remarks', 'Exc Remarks',
               'TELE Remarks', 'Coutresy call', 'Happy call', 'Booked', 'Sold', 'Month End Review', 'Status',
               'Last followup on', 'District', 'Customer ID', 'Enquiry Created On', 'Enquiry Created By',
               'Next followup on', 'Last customer remark', 'Last SM remark', 'Last ASM remark', 'Last MIS remark', 'Showroom',
               'Home visit on', 'Enq Status', 'Enq Type', 'Division'
          );
          //Loop Heading
          $rowNumberH = 1;
          $colH = 'A';
          ...
          if (isset($data['data']) && !empty($data['data'])) {
               foreach ($data['data'] as $key => $value) {
                    ...
                    $objPHPExcel->getActiveSheet()->setCellValue('AD' . $row, (isset($value['enq_last_foll_date']) && !empty($value['enq_last_foll_date'])) ? date('j M Y', strtotime($value['enq_last_foll_date'])) : '');
                    ...
                    $objPHPExcel->getActiveSheet()->setCellValue('AG' . $row, !empty($value['enq_added_on']) ? date('j M Y', strtotime($value['enq_added_on'])) : '');
                    $objPHPExcel->getActiveSheet()->setCellValue('AH' . $row, $value['createdby_first_name']);
                    $objPHPExcel->getActiveSheet()->setCellValue('AI' . $row, !empty($value['enq_next_foll_date']) ? date('j M Y', strtotime($value['enq_next_foll_date'])) : '');
                    ...
                    $objPHPExcel->getActiveSheet()->setCellValue('AO' . $row, !empty($value['enq_home_visit_date']) ? date('j M Y', strtotime($value['enq_home_visit_date'])) : '');
     ...
     function exportEnquiresold()
     {
          ...
          $objPHPExcel->getActiveSheet()->setTitle('Enquires report');
          $heading = array(
               'SL', 'Enquiry Date', 'Customer Name', 'Phone Number', 'Profession', 'Location', 'Brand', 'Model Interested',
               ...
               'Last followup on', 'Next followup on', 'District'
          );
          //Loop Heading
          $rowNumberH = 1;
          $colH = 'A';
     ...
     function exportHomeVisit()
     {
          ...
          $heading = array(
               'Enq Number', 'Customer', 'Customer Contact No', 'Customer status', 'Request on', 'Place',
               'Staff', 'Department', 'Travel With', 'Vehicle',
               'Reg No', 'Out KM', 'In KM', 'In Date', 'Approved', 'Discussion Time', 'Met the Customer with Family'
          );
          ...
          if (!empty($data)) {
               foreach ($data as $key => $value) {
                    ...
                    $objPHPExcel->getActiveSheet()->setCellValue('E' . $row, date('d-m-Y', strtotime($value['hmv_date'])));
     ...
     function exportChecklist()
     {
          ...
          if (!empty($data)) {
               foreach ($data as $key => $value) {
                    ...
                    $objPHPExcel->getActiveSheet()->setCellValue('D' . $row, $value['stock_created_date'] ? date('M d, Y', strtotime($value['stock_created_date'])) : '');
