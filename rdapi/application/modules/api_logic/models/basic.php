...
class basic extends CI_Model
{
     ...
     function getbmv()
     {
          $selectArray = array(
               ...
               $this->tbl_seo_pages . '.seop_name',
               $this->tbl_seo_pages . '.seop_page_title',
               $this->tbl_seo_pages . '.seop_meta_desc',
               $this->tbl_seo_cms . '.seocms_content'
          );
          return $this->db->select($selectArray)->join($this->tbl_brands, $this->tbl_brands . '.brd_id = ' . $this->tbl_variant . '.var_brand_id')
               ->join($this->tbl_model, $this->tbl_model . '.mod_id = ' . $this->tbl_variant . '.var_model_id', 'LEFT')
               ->join($this->tbl_seo_pages, $this->tbl_seo_pages . '.seop_brand = ' . $this->tbl_brands . '.brd_id', 'LEFT')
               ->join($this->tbl_seo_cms, $this->tbl_seo_cms . '.seocms_brand = ' . $this->tbl_brands . '.brd_id', 'LEFT')
               ->order_by('brd_title')->get($this->tbl_variant)->result_array();
     }
     ...
     function getCarsWithPageNew($limit = 0, $page = 0)
     {
          $selectFields = array(
               ...
               $this->tbl_products . '.prd_km_run',
               $this->tbl_products . '.prd_variant',
               $this->tbl_products . '.prd_year',
               $this->tbl_products . '.prd_color',
               $this->tbl_products . '.prd_wrapp_color',
               $this->tbl_products . '.prd_mileage',
               $this->tbl_products . '.prd_owner',
               $this->tbl_products . '.prd_engine_cc',
               $this->tbl_products . '.prd_price',
               $this->tbl_products . '.prd_model',
               $this->tbl_products . '.prd_insurance_validity',
               $this->tbl_products . '.prd_insurance_idv',
               $this->tbl_products . '.prd_insurance_type',
               $this->tbl_products . '.prd_fual',
               $this->tbl_products . '.prd_fual_old',
               $this->tbl_products . '.prd_brand',
               $this->tbl_products . '.prd_new_arrivals',
               $this->tbl_products . '.prd_desc',
               $this->tbl_products . '.prd_rd_mini',
               $this->tbl_products . '.prd_popular',
               $this->tbl_products . '.prd_booked',
               $this->tbl_products . '.prd_soled',
               $this->tbl_products . '.prd_latest',
               $this->tbl_products . '.prd_loan_avail',
               $this->tbl_products . '.prd_page_title',
               $this->tbl_products . '.prd_meta_desc',
               $this->tbl_products . '.prd_video',
               $this->tbl_products . '.prd_show_price',
               $this->tbl_products . '.prd_warranty_upto',
               $this->tbl_products . '.prd_transmission',
               $this->tbl_products . '.prd_arai_mileage',
               $this->tbl_products . '.prd_acc_ftd',
               $this->tbl_products . '.prd_or_road_price',
               $this->tbl_products . '.prd_featured',
               $this->tbl_products . '.prd_status',
               $this->tbl_vehicle_colors . '.vc_color AS prd_color',
               'prd_rd_mini', 'prd_popular', 'prd_latest', 'prd_soled',
               'prd_booked', 'brd_title', 'mod_title', 'mod_is_ev', 'mod_veh_type',
               'var_variant_name', 'prd_year', 'prd_owner AS ownership', 'IF(prd_show_price = 1, prd_price, 0) AS price',
               'prd_km_run AS kms',
               "(CASE WHEN prd_fual = 1 THEN 'Petrol'
                    WHEN prd_fual = 2 THEN 'Diesel'
                    WHEN prd_fual = 3 THEN 'Gas'
          ...
          if (!empty($productsArray['mdata'])) {
               foreach ($productsArray['mdata'] as $key => $value) {
                    ...
                    //$productsArray = $p;
                    $this->db->where('prd_id', $value['prd_id'])->update($this->tbl_products, array('prd_sync' => 1));
                    //debug($productsArray);
               }
          }
          ///e
