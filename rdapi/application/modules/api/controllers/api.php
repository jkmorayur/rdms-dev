...
class Api extends App_Controller
{
     ...
     function get_cars_new($id = '')
     {
          ...
          $data['pagination']['limit'] = $limit;
          $data['pagination']['totalRow'] = 0; //number_format($all);
          echo json_encode($data);
     }
     ...
     function rdpromotions()
     {
          generate_log(array(
               'log_title' => 'promotions',
               'log_desc' => serialize($_POST),
               'log_controller' => 'rd-promotions',
               'log_action' => 'C',
               'log_ref_id' => 0,
               'log_added_by' => 2020202020
          ));
          $data = $_POST;
          $event =  (isset($data['eve_event']) && !empty($data['eve_event'])) ? $data['eve_event'] : 0;
          if ($event == 22) { //Investment
               $this->api->createEventEnquiry($data);
               echo json_encode('We will touch you soon!');
          } else if ($event == 21) { //Purchase
               unset($data['mailtitle']);
               $this->api->createEventEnquiry($data);
               echo json_encode('We will touch you soon!');
          } else { // NRI-Fest
               if (isset($_FILES['eve_file'])) {
                    $this->load->library('upload');
                    $config['upload_path'] = '../rdapi/assets/uploads/nri_fest/';
                    $config['allowed_types'] = '*';
                    $config['encrypt_name'] = TRUE;
                    $this->upload->initialize($config);
                    $this->upload->do_upload('eve_file');
                    $updata = $this->upload->data();
                    $data['eve_file'] = isset($updata['file_name']) ? $updata['file_name'] : '';
               }
               $data['eve_type'] = 1;
               $data['eve_vehicle'] = 0;
               $this->api->rdpromotions($data);
               echo json_encode('We will touch you soon!');
          }
     }
     ...
     public function subscribe()
     {
          $params = json_decode(file_get_contents('php://input'), TRUE);
          //debug($params);
          $res = $this->api->saveSubscription($params);
          if ($res) {
               echo json_encode(array(
                    'status' => TRUE,
