...
//                            ->where('MONTH(' . $this->tbl_register_master . '.vreg_added_on) = MONTH(CURRENT_DATE())')
          ...
          if ($this->uid > 0) {
               if (check_permission('booking', 'showbooking')) {
                    ...
                    $return['booked'] = $this->db->join($this->tbl_vehicle_booking_confirmations, $this->tbl_vehicle_booking_confirmations .
                                   ...
                                   ->where_in($this->tbl_vehicle_booking_master . '.vbk_status',
                                        array(vehicle_booked))->count_all_results($this->tbl_vehicle_booking_master);
                    ...
                    $return['bookConfirmed'] = $this->db->where_in($this->tbl_vehicle_booking_master . '.vbk_status',
                                   array(confm_book, rfi_loan_approved, dc_ready_to_del))->count_all_results($this->tbl_vehicle_booking_master);
               }
     ...
     function getVehicleColors($id = '') {
          if ($id) {
               return $this->db->order_by('vc_sort_order', 'asc')->where('vc_id', $id)->get($this->tbl_vehicle_colors)->row_array();
          } else {
               return $this->db->order_by('vc_sort_order', 'asc')->get($this->tbl_vehicle_colors)->result_array();
          }
     }
