  ...
  class reports_1 extends App_Controller {
       ...
       public function __construct() {
           ...
           $this->load->model('enquiry/enquiry_model', 'enquiry');
            ...
            $this->load->model('booking_new/booking_model', 'booking');
...
//            debug($data['searchResult']);
       ...
       function exportEnquires() {
            ...
            $objPHPExcel = new PHPExcel();
            $objPHPExcel->getActiveSheet()->setTitle('Enquires report');
            $heading = array('SL', 'Date', 'Customer Name', 'Phone Number', 'Profession', 'Location', 'Brand', 'Model Interested',
...
// exit;
       ...
       function booked_stock_report() {//STOCK STATUS AS ON 23-08-2021
            if ($this->input->is_ajax_request()) {
       ...
       function purchase_report() {//PURCHASE REPORT AS ON 23-08-2021
          //debug(333);
            if ($this->input->is_ajax_request()) {
                 // debug($this->input->post('month'));
                 $filterData = $this->input->post();
...
// exit;
       ...
       function summary_enq() {//STOCK STATUS AS ON 23-08-2021
            if ($this->input->is_ajax_request()) {
                 // $filterData = $this->input->post();
                 $shrm = $this->input->post('showroom');
...
//                 }
       ...
       function purchase_new_live_enqs() {//2 cmplt
            if ($this->input->is_ajax_request()) {
                 $showroom = $this->input->post('showroom');
                 //  debug($showroom);
       ...
       function enq_model_wise_analysis() {//tableContent
            if ($this->input->is_ajax_request()) {
       ...
       function summary_enq_purchase() {//STOCK STATUS AS ON 23-08-2021
            if ($this->input->is_ajax_request()) {
                 // $filterData = $this->input->post();
                 $shrm = $this->input->post('showroom');
                 $month = $this->input->post('month');
...
//                 $month=9;
       ...
       function detailed_walk_in_report() {//detailedWalikInReport
            if ($this->input->is_ajax_request()) {
                 $showroom = $this->input->post('showroom');
                 //  $showroom = 1;
