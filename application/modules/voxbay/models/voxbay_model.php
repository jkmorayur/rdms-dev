<?php
  ...
  class voxbay_model extends CI_Model {
       ...
       public function pendingCalls($postDatas) {
            ...
            if ($this->usr_grp == 'TC' || $this->usr_grp == 'SE') { $this->db->where('ccb_force_assign', $this->uid); }
       ...
       function getOutboundByNumber($number) {
            ...
            $this->db->select(array(
                ...
                "CONCAT('http://45.249.170.209:8080/content/callrecordings/'," . $this->tbl_callcenterbridging_outgoing . '.ccbo_recording_URL,' . "'.wav') AS ccbo_recording_URL",
     ...
     public function getAllConnectedOutCall($postDatas) {
          ...
          $this->db->limit($rowperpage, $row);
          $this->db->select(array('ccbo_id','ccbo_destination_user','ccbo_callerid', 'ccbo_status', 'ccbo_destination', "DATE_FORMAT(ccbo_date, '%M %d %Y %h %i') AS ccbo_date",
              "CONCAT('http://45.249.170.209:8080/content/callrecordings/'," . $this->tbl_callcenterbridging_outgoing . '.ccbo_recording_URL,' . "'.wav') AS ccbo_recording_URL"
          ));
          //$this->db->where('ccb_callStatus_id', VB_CONNECTED)->where_not_in('ccb_category', $this->hideCategory);
          if (!empty($dataF)) {
