...
class registration extends App_Controller
{
     ...
     public function export_excel()
     {
          ...
          $heading = array(
               'Date', 'Customer', 'Customer Contact No', 'Location', 'Profession', 'Brand', 'Model', 'Variant',
               'Budget', 'Year', 'KM', 'Finance', 'Exchange', 'Department', 'Mode of contact', 'Assigned by', 'Assigned to', 'Customer remark', 'Executive remarks'
          );
     ...
     function matchingInquiry()
     {
          ...
          if (empty($isDroppedCase)) {
               die(json_encode(array('status' => 'fail', 'msg' => 'Enquiry not found', 'usr_id' => '', 'se' => '', 'isdrop' => 0, 'regHistory' => $reghistory)));
          } else if ($isDroppedCase['enq_current_status'] == 3) {
               $se = isset($isDroppedCase['usr_first_name']) ? $isDroppedCase['usr_first_name'] : '';
               $trackCard = ' <a target="_blank" href="' . site_url('enquiry/printTrackCard/' . encryptor($isDroppedCase['enq_id'])) . '"><i title="Track card" class="fa fa-list"></i></a>';
               $msg = 'Enquiry dropped Sales Officer ' . $se . ' Track card : ' . $trackCard;
               die(json_encode(array('status' => 'fail', 'msg' => $msg, 'usr_id' => '', 'se' => '', 'isdrop' => 1, 'regHistory' => $reghistory)));
          } else {
               $duplicate = $this->registration->matchingInquiry($this->input->post('phoneNo'));
               if (!empty($duplicate)) {
                    $se = isset($duplicate['usr_first_name']) ? $duplicate['usr_first_name'] : '';
                    $userId = isset($duplicate['usr_id']) ? $duplicate['usr_id'] : '';
                    $trackCard = ' <a target="_blank" href="' . site_url('enquiry/printTrackCard/' . encryptor($duplicate['enq_id'])) . '"><i title="Track card" class="fa fa-list"></i></a>';
                    $msg = 'Inquiry already associated with sales executive';
                    if ($this->usr_grp == 'AD' || $this->usr_grp == 'TC' || (check_permission('registration', 'knowenqtrackcard'))) {
                         $msg = 'Inquiry already associated with sales executive ' . $se . ' Track card : ' . $trackCard;
                    }
                    echo json_encode(array('status' => 'success', 'msg' => $msg, 'usr_id' => $userId, 'se' => $se, 'isdrop' => 0, 'regHistory' => $reghistory));
               } else {
                    echo json_encode(array(
                         'status' => 'fail', 'msg' => 'No inquiry already associated with this contact number',
     ...
     public function export_excel_event($type = 'lky-drw')
     {
          ...
          if ($type == 'evnt') {
               ...
               $heading = array(
                    'Entry date', 'Customer name', 'Contact', 'Location', 'Email', 'Event', 'Reg no', 'Vehicle', 'Punched by', 'Comments',
                    'Referal name', 'Referal mobile NRI', 'Referal mobile IN', 'Referal location'
               );
