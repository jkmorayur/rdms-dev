          ...
          <div class="col-md-12 col-sm-12 col-xs-12">
               <div class="x_panel">
                    ...
                    <div class="x_content">
                         ...
                         <form action="<?php echo site_url($controller . '/index?' . $_SERVER['QUERY_STRING']); ?>" method="get">
                              <table>
                                   <tr>
                                        <td>
                                             <select data-url="<?php echo site_url('enquiry/bindModel'); ?>" data-bind="cmbEvModel" data-dflt-select="Select Model" class="select2_group form-control cmbBrand bindToDropdown" name="vreg_brand" id="vreg_brand">
                                                  <option value="">Select Brand</option>
                                                  <?php if (!empty($brand)) {
                                                       foreach ($brand as $key => $value) { ?>
                                                            <option <?php echo (isset($_GET['vreg_brand']) && $_GET['vreg_brand'] == $value['brd_id']) ? "selected" : ''; ?> value="<?php echo $value['brd_id']; ?>"><?php echo $value['brd_title']; ?></option>
                                                  <?php }
                                                  }
                                                  ?>
                                             </select>
                                        ...
                                        <td>
                                             <select data-url="<?php echo site_url('enquiry/bindVarient'); ?>" data-bind="cmbEvVariant" data-dflt-select="Select Variant" class="select2_group form-control cmbEvModel bindToDropdown" name="vreg_model" id="vreg_model">
                                                  <option value="">Select Model</option>
                                                  <?php foreach ((array) $model as $key => $value) { ?>
                                                       <option <?php echo (isset($_GET['vreg_model']) && ($value['mod_id'] == $_GET['vreg_model'])) ? 'selected="selected"' : ''; ?> value="<?php echo $value['mod_id']; ?>"><?php echo $value['mod_title']; ?></option>
                                                  <?php } ?>
                                             </select>
                                        </td>
                                        <td>
                                             <select class="cmbEvVariant select2_group form-control" name="vreg_varient" id="vreg_varient">
                                                  <option value="">Select Variant</option>
                                                  <?php foreach ((array) $variant as $key => $value) { ?>
                                                       <option <?php echo (isset($_GET['vreg_varient']) && ($value['var_id'] == $_GET['vreg_varient'])) ? 'selected="selected"' : ''; ?> value="<?php echo $value['var_id']; ?>"><?php echo $value['var_variant_name']; ?></option>
                                                  <?php } ?>
                                             </select>
                                        </td>
                                   </tr>
                                   <tr>
                                        <td>
                                             <input autocomplete="off" name="enq_date_from" type="text" class="dtpDatePickerDMY form-control col-md-7 col-xs-12" placeholder="Date from" value="<?php echo $enq_date_from; ?>" />
                                        </td>
                                        <td style="padding-left: 10px;">
                                             <input autocomplete="off" name="enq_date_to" type="text" class="dtpDatePickerDMY form-control col-md-7 col-xs-12" placeholder="Date to" value="<?php echo $enq_date_to; ?>" />
                                        </td>
                                        <td style="padding-left: 10px;">
                                             <select style="float: left;width: auto;" class="muliSelectCombo select2_group form-control" name="mode[]" multiple="multiple">
                                                  <option value="0">All Mode of enquiry</option>
                                                  <?php foreach (unserialize(MODE_OF_CONTACT) as $sts => $stsName) { ?>
                                                       <option <?php echo (@in_array($sts, $mode)) ? 'selected="selected"' : ''; ?> value="<?php echo $sts; ?>"><?php echo $stsName ?></option>
                                                  <?php } ?>
                                             </select>
                                        </td>
                                             ...
                                             <td style="padding-left: 10px;">
                                                  <select multiple="multiple" style="float: left;width: auto;" class="muliSelectCombo select2_group form-control cmbSalesExecutives" name="executive[]">
                                                       ...
                                                       ?>
                                                                 ...
                                                                 ?>
                                                                 <option <?php echo (@in_array($value['usr_id'], $executive)) ? 'selected="selected"' : ''; ?> value="<?php echo $value['usr_id']; ?>"><?php echo $value['usr_first_name']; ?></option>
                                                       <?php
                                                            }
                                                       }
                                                       ?>
                                        ...
                                        <?php } ?>
                                        <td>
                                             <input autocomplete="off" name="search" type="text" class="form-control col-md-7 col-xs-12" placeholder="Search by phone, name" value="<?php echo isset($_GET['search']) ? $_GET['search'] : ''; ?>" />
                                        </td>
                                   </tr>
                                   <tr>
                                        <td>
                                        ...
                                        <td>
                                             <select class="select2_group form-control" name="vreg_call_type" id="vreg_call_type">
                                                  <option value="">Select lead type</option>
                                                  <?php
                                                  foreach (unserialize(CALL_TYPE) as $key => $value) {
                                                  ?><option <?php echo (isset($_GET['vreg_call_type']) && ($_GET['vreg_call_type'] == $key)) ? 'selected="selected"' : ''; ?> value="<?php echo $key; ?>"><?php echo $value; ?></option><?php
                                                                                                                                                                                                                                      }
                                                                                                                                                                                                                                           ?>
                                             </select>
                                        </td>
                                        ...
                                        <td>
                                             <a href="<?php echo site_url($controller . '/add'); ?>" class="btn btn-round btn-primary" style="margin-bottom: 10px;float: right;">New vehicle register</a>
                                        </td>
                                   </tr>
                              </table>
                         </form>
                         ...
                         <div style="width: 100%;overflow-x: scroll;overflow-y: hidden;">
                              <table id="datatable" class="table table-striped table-bordered">
                                   ...
                                   <tbody>
                                        ...
                                        ?>
                                             <tr style="<?php echo ($value['vreg_status'] == 0) ? 'color: #fff;background-color: red;' : ''; ?>" data-url="<?php echo site_url($controller . '/view/' . encryptor($value['vreg_id'])); ?>">
                                                  ...
                                                  <td>
                                                       <a style="<?php echo ($value['vreg_status'] == 0) ? 'color: #fff;background-color: red;' : ''; ?>" <?php echo 'href="https://api.whatsapp.com/send?phone=' . $value['vreg_cust_phone'] . '"'; ?>><?php echo $value['vreg_cust_phone']; ?></a>
                                                  </td>
                                                  ...
                                                  <td class="<?php echo $trVOE; ?>"><?php echo date('j M Y', strtotime($value['vreg_added_on'])); ?></td>
                                                  <td class="<?php echo $trVOE; ?>"><?php echo $value['assign_usr_first_name'] . ' ' . $value['assign_usr_last_name']; ?></td>
                                                  <td class="<?php echo $trVOE; ?>">
