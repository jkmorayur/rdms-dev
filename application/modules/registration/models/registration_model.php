...
class registration_model extends CI_Model
{
          ...
          if (!empty($oldAssign) && isset($datas['vreg_assigned_to'])) {
               if ($oldAssign != $datas['vreg_assigned_to']) { //Assigned to changed
                    ...
                    $this->db->where(array('vreg_id' => $_POST['regMaster']))->update($this->tbl_register_master, array(
                         ...
                         'vreg_assigned_to' => $_POST['assignedTo'], 'vreg_added_by' => $this->uid, 'vreg_status' => 1
                    ));
               }
          }
     ...
     function matchingRegister($phoneNo)
     {
          if (!empty($phoneNo)) {
               ...
               $this->db->select($this->tbl_register_master . '.vreg_last_action,' .
                    $this->tbl_register_master . '.vreg_cust_name,' . $this->tbl_register_master . '.vreg_cust_name,' .
                    ...
                    'fwon.usr_id AS fwon_usr_id, fwon.usr_first_name AS fwon_usr_name, ' . $this->tbl_district_statewise . '.*')
